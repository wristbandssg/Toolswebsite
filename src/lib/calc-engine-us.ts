/**
 * United States federal + state income tax calculators.
 *
 * Split out of calc-engine.ts (which had grown to 4000+ lines covering all
 * 50 states) so each country's tax logic lives in its own file — see
 * calc-engine.ts for how country files are merged into the single
 * `customCalculators` registry the rest of the app uses, and
 * calc-engine-types.ts for the shared CustomCalculator/CalcInputValues
 * types every country file (this one, calc-engine-uk.ts, ...) implements
 * against.
 *
 * Every calculator here follows the same shape: gross pay -> pre-tax
 * deductions -> federal income tax (2026 IRS brackets + standard deduction)
 * -> FICA (Social Security + Medicare) -> that state's own income tax logic
 * -> totals. See each state's own comment block for its specific tax
 * mechanism, sources, and documented simplifications.
 */
...
export const usCustomCalculators: Record<string, CustomCalculator> = {
