// One-time (but safe to re-run) setup script: creates the South Dakota
...
//
// Like Nevada, South Dakota levies NO state income tax on wages, so this
// tool's breakdown always shows a $0 South Dakota State Income Tax line. The
// math lives in code, not the database: see
// `customCalculators["south-dakota-tax-calculator"]` in
...
// it's a direct alias of the Nevada federal+FICA-only calculator. This
// script only wires up the Tool row so the public page has a title, input
// form, and content around that calculation.
//
// HOW TO RUN
//   npx tsx prisma/create-south-dakota-tax-tool.ts
// or
//   npm run db:create-south-dakota-tool

import { PrismaClient, Prisma } from "@prisma/client";

const prisma = new PrismaClient();

const SLUG = "south-dakota-tax-calculator";
...
async function main() {
  ...
  const calcResults = [
    ...
    { key: "stateIncomeTax", label: "South Dakota State Income Tax", format: "currency" },
  ...
  const instructions =
    "Whether you're looking for a South Dakota income tax calculator, a paycheck tax calculator, a payroll tax " +
    "calculator, or just a general tax calculator for South Dakota, this tool covers it: it works out federal " +
    "income tax, Social Security, Medicare, and South Dakota's state income tax (always zero) from your salary, " +
    "all in one place.\n\n" +
    ...
    "breakdown: gross pay, federal income tax, Social Security tax, Medicare tax, South Dakota state income tax " +
    "(always $0 — South Dakota has no state income tax), total deductions, and your estimated take-home pay, " +
    "both per paycheck and for the year.\n\n" +
    "South Dakota is one of nine U.S. states that levy no state income tax on wages at all, so its part of the " +
    "breakdown is simple: it's always $0, no matter your income, filing status, or where in the state you live. " +
    "South Dakota is also frequently cited for having one of the lowest overall state and local tax burdens in " +
    "the country, and it levies no corporate income tax either.\n\n" +
  ...
  const assumptions =
    ...
    "South Dakota has no state income tax and no local income taxes, so that part of the breakdown needs no " +
    "assumptions — it's always $0 for every filer, at every income level, statewide. This calculator doesn't " +
    "model any other South Dakota state or local taxes (such as sales or property tax), since none of them come " +
    "out of a paycheck.\n\n" +
    "South Dakota's lack of a state income tax means its portion of this estimate can't drift from reality the " +
    "way a bracket- or deduction-based state's can — the only real variability here is on the federal side.\n\n" +
  ...
  const examples =
    ...
    "after federal income tax, Social Security, and Medicare. South Dakota adds no state income tax on top of " +
    "that.\n\n" +
    "Because South Dakota has no state income or payroll tax, the only difference between this estimate and a " +
  ...
  const faq = [
    {
      ...
      answer:
        "No. South Dakota is one of nine U.S. states with no state income tax, so nothing is withheld from your " +
        "paycheck for state income tax — your take-home pay is reduced only by federal income tax and FICA " +
        "(Social Security and Medicare).",
    },
    {
      question: "Why doesn't South Dakota have an income tax?",
      answer:
        "South Dakota has chosen not to levy a personal income tax and, notably, also levies no corporate " +
        "income tax — it's regularly cited as one of the states with the lowest overall state and local tax " +
        "burden in the country.",
    },
    {
      question: "What taxes ARE taken out of a South Dakota paycheck?",
      answer:
        ...
        "on your filing status). There's no state or local income tax in South Dakota.",
    },
    {
      ...
      answer:
        ...
        "and this calculator includes all of it: federal income tax, Social Security, and Medicare, plus South " +
        "Dakota's (zero) state payroll tax, all in the same breakdown.",
    },
    {
      question: "Is there a South Dakota salary tax calculator I can use for any pay frequency?",
      answer:
        "Yes — this calculator works as a South Dakota salary tax calculator for weekly, biweekly, " +
        "semi-monthly, monthly, or annual pay. Enter your annual salary and choose your pay frequency above, " +
        "and it recalculates federal income tax, Social Security, and Medicare for that schedule automatically.",
    },
    {
      question: "What is my take-home pay in South Dakota?",
      answer:
        "Your take-home pay is your gross salary minus federal income tax, Social Security, Medicare, and any " +
        "deductions you enter. Because South Dakota has no state income tax, nothing further is subtracted for " +
        "state tax, so South Dakota workers typically keep more of their gross pay than workers in states that " +
        "do tax income.",
    },
    {
    ...
    },
    {
      ...
      answer:
        ...
        "from your wages before federal income tax and FICA are calculated, which lowers your total tax — so " +
        "your take-home pay drops by less than the full deduction amount.",
    },
  ];
  ...
  const toolContent = {
    ...
    description:
      "This South Dakota income tax calculator and paycheck tax calculator shows you, in one place, exactly " +
      "what's withheld from your paycheck and what you take home. Use it as a general tax calculator for South " +
      "Dakota, a South Dakota payroll tax calculator for federal withholding and FICA, or a salary tax " +
      "calculator for any pay frequency — since South Dakota charges no state income tax, more of every " +
  ...
  const seoMetaContent = {
    contentType: "tool",
    metaTitle: "South Dakota Income Tax Calculator (2026) — Paycheck & Payroll Tax",
    metaDescription:
      "Free South Dakota income tax calculator and paycheck tax calculator. Estimate federal income tax, " +
      "payroll tax (FICA), and take-home pay — South Dakota has no state income tax.",
  ...
  console.log(
    "Open it in /admin/tools, review it, then set Status to Published when you're happy with it. " +
      "Its live URL will be /tools/" + SLUG + ". Once published, link it to South Dakota's row in " +
