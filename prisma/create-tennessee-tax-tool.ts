// One-time (but safe to re-run) setup script: creates the Tennessee Income
...
//
// Like Nevada, Tennessee levies NO state income tax on wages, so this
// tool's breakdown always shows a $0 Tennessee State Income Tax line. The
// math lives in code, not the database: see
// `customCalculators["tennessee-tax-calculator"]` in `src/lib/calc-engine.ts`
// for the federal income tax (2026 IRS brackets + standard deduction) and
// FICA (Social Security + Medicare) calculation — it's a direct alias of the
// Nevada federal+FICA-only calculator. This script only wires up the Tool
// row so the public page has a title, input form, and content around that
// calculation.
//
// HOW TO RUN
//   npx tsx prisma/create-tennessee-tax-tool.ts
// or
//   npm run db:create-tennessee-tool

import { PrismaClient, Prisma } from "@prisma/client";

const prisma = new PrismaClient();

const SLUG = "tennessee-tax-calculator";
...
async function main() {
  ...
  const calcResults = [
    ...
    { key: "stateIncomeTax", label: "Tennessee State Income Tax", format: "currency" },
  ...
  const instructions =
    "Whether you're looking for a Tennessee income tax calculator, a paycheck tax calculator, a payroll tax " +
    "calculator, or just a general tax calculator for Tennessee, this tool covers it: it works out federal " +
    "income tax, Social Security, Medicare, and Tennessee's state income tax (always zero) from your salary, " +
    "all in one place.\n\n" +
    ...
    "breakdown: gross pay, federal income tax, Social Security tax, Medicare tax, Tennessee state income tax " +
    "(always $0 — Tennessee has no state income tax), total deductions, and your estimated take-home pay, both " +
    "per paycheck and for the year.\n\n" +
    "Tennessee levies no tax on wage income, and hasn't for some time — it went a step further in 2021 by " +
    "fully repealing the old \"Hall Tax,\" which had taxed interest and dividend income even though wages were " +
    "never covered by it. Today Tennessee taxes neither wages nor investment income at the state level.\n\n" +
  ...
  const assumptions =
    ...
    "Tennessee has no state income tax and no local income taxes, so that part of the breakdown needs no " +
    "assumptions — it's always $0 for every filer, at every income level, statewide. Since 2021, that " +
    "zero also covers interest and dividend income (formerly taxed under the Hall Tax), though this calculator " +
    "only models wage income to begin with.\n\n" +
    "This calculator only reflects taxes withheld from a paycheck — it doesn't model any other Tennessee state " +
    "or local taxes, such as sales tax, since those aren't deducted from wages.\n\n" +
  ...
  const examples =
    ...
    "after federal income tax, Social Security, and Medicare. Tennessee adds no state income tax on top of " +
    "that.\n\n" +
    "Because Tennessee has no state income or payroll tax, the only difference between this estimate and a " +
  ...
  const faq = [
    {
      ...
      answer:
        "No. Tennessee is one of nine U.S. states with no state income tax, so nothing is withheld from your " +
        "paycheck for state income tax — your take-home pay is reduced only by federal income tax and FICA " +
        "(Social Security and Medicare).",
    },
    {
      question: "Did Tennessee ever tax any income?",
      answer:
        "Tennessee used to tax interest and dividend income under a law known as the \"Hall Tax,\" but it never " +
        "applied to wages, and it was fully phased out and repealed at the start of 2021. Since then Tennessee " +
        "has taxed neither wages nor investment income at the state level.",
    },
    {
      question: "What taxes ARE taken out of a Tennessee paycheck?",
      answer:
        ...
        "on your filing status). There's no state or local income tax in Tennessee.",
    },
    {
      ...
      answer:
        ...
        "Tennessee's (zero) state payroll tax, all in the same breakdown.",
    },
    {
      question: "Is there a Tennessee salary tax calculator I can use for any pay frequency?",
      answer:
        "Yes — this calculator works as a Tennessee salary tax calculator for weekly, biweekly, semi-monthly, " +
    ...
    {
      question: "What is my take-home pay in Tennessee?",
      answer:
        "Your take-home pay is your gross salary minus federal income tax, Social Security, Medicare, and any " +
        "deductions you enter. Because Tennessee has no state income tax, nothing further is subtracted for " +
        "state tax, so Tennessee workers typically keep more of their gross pay than workers in states that do " +
        "tax income.",
    },
    {
  ...
  const toolContent = {
    ...
    description:
      "This Tennessee income tax calculator and paycheck tax calculator shows you, in one place, exactly what's " +
      "withheld from your paycheck and what you take home. Use it as a general tax calculator for Tennessee, a " +
      "Tennessee payroll tax calculator for federal withholding and FICA, or a salary tax calculator for any pay " +
      "frequency — since Tennessee charges no state income tax, more of every paycheck stays with you.",
  ...
  const seoMetaContent = {
    contentType: "tool",
    metaTitle: "Tennessee Income Tax Calculator (2026) — Paycheck & Payroll Tax",
    metaDescription:
      "Free Tennessee income tax calculator and paycheck tax calculator. Estimate federal income tax, payroll " +
      "tax (FICA), and take-home pay — Tennessee has no state income tax.",
  ...
  console.log(
    "Open it in /admin/tools, review it, then set Status to Published when you're happy with it. " +
      "Its live URL will be /tools/" + SLUG + ". Once published, link it to Tennessee's row in " +
