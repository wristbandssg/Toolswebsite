// One-time (but safe to re-run) setup script: creates the Wyoming Income
...
//
// Like Nevada, Wyoming levies NO state income tax on wages, so this tool's
// breakdown always shows a $0 Wyoming State Income Tax line. The math lives
// in code, not the database: see
// `customCalculators["wyoming-tax-calculator"]` in `src/lib/calc-engine.ts`
...
//
// HOW TO RUN
//   npx tsx prisma/create-wyoming-tax-tool.ts
// or
//   npm run db:create-wyoming-tool

import { PrismaClient, Prisma } from "@prisma/client";

const prisma = new PrismaClient();

const SLUG = "wyoming-tax-calculator";
...
async function main() {
  ...
  const calcResults = [
    ...
    { key: "stateIncomeTax", label: "Wyoming State Income Tax", format: "currency" },
  ...
  const instructions =
    "Whether you're looking for a Wyoming income tax calculator, a paycheck tax calculator, a payroll tax " +
    "calculator, or just a general tax calculator for Wyoming, this tool covers it: it works out federal income " +
    "tax, Social Security, Medicare, and Wyoming's state income tax (always zero) from your salary, all in one " +
    "place.\n\n" +
    ...
    "breakdown: gross pay, federal income tax, Social Security tax, Medicare tax, Wyoming state income tax " +
    "(always $0 — Wyoming has no state income tax), total deductions, and your estimated take-home pay, both " +
    "per paycheck and for the year.\n\n" +
    "Wyoming is one of nine U.S. states with no state income tax, and it's also widely known for having one of " +
    "the lowest overall state and local tax burdens in the country, plus no corporate income tax on top of " +
    "that. None of that changes how federal income tax, Social Security, or Medicare are worked out — those are " +
    "calculated the same way for a Wyoming paycheck as anywhere else in the country — it just means the state " +
    "tax line on your paycheck stays at $0.\n\n" +
  ...
  const assumptions =
    ...
    "Wyoming has no state income tax and no local income taxes, so that part of the breakdown needs no " +
    "assumptions — it's always $0 for every filer, at every income level, statewide.\n\n" +
    "This calculator covers wage paycheck withholding only. It doesn't attempt to model Wyoming's broader tax " +
    "mix (such as property or severance tax revenue), since none of that is withheld from a paycheck and none " +
    "of it affects the take-home figures shown here.\n\n" +
  ...
  const examples =
    ...
    "after federal income tax, Social Security, and Medicare. Wyoming adds no state income tax on top of " +
    "that.\n\n" +
    "Because Wyoming has no state income or payroll tax, the only difference between this estimate and a " +
    "paycheck in a state with income tax is that extra state withholding line — everything else (federal " +
    "income tax, Social Security, Medicare) is calculated the same way nationwide. Combined with Wyoming's low " +
    "overall tax burden generally, this is part of why take-home pay in Wyoming tends to stretch further than " +
    "the same gross salary in many other states.";

  const faq = [
    {
      question: "Does Wyoming have a state income tax?",
      answer:
        "No. Wyoming is one of nine U.S. states with no state income tax, so nothing is withheld from your " +
        "paycheck for state income tax — your take-home pay is reduced only by federal income tax and FICA " +
        "(Social Security and Medicare).",
    },
    {
      question: "Why doesn't Wyoming have an income tax?",
      answer:
        "Wyoming has long kept its overall tax burden low, and it also levies no corporate income tax. It's " +
        "consistently ranked among the states with the lowest combined state and local tax burden in the " +
        "country, and no state income tax on wages is part of that picture.",
    },
    {
      question: "What taxes ARE taken out of a Wyoming paycheck?",
      answer:
        ...
        "on your filing status). There's no state or local income tax in Wyoming.",
    },
    {
      ...
      answer:
        ...
        "this calculator includes all of it: federal income tax, Social Security, and Medicare, plus Wyoming's " +
        "(zero) state payroll tax, all in the same breakdown.",
    },
    {
      question: "Is there a Wyoming salary tax calculator I can use for any pay frequency?",
      answer:
        "Yes — this calculator works as a Wyoming salary tax calculator for weekly, biweekly, semi-monthly, " +
    ...
    {
      question: "What is my take-home pay in Wyoming?",
      answer:
        "Your after-tax pay is your gross salary minus federal income tax, Social Security, Medicare, and any " +
        "deductions you enter. Because Wyoming has no state income tax, nothing further is subtracted for state " +
        "tax, so Wyoming workers typically keep more of their gross pay than workers in states that do tax " +
        "income.",
    },
    {
    ...
    {
      question: "Does Wyoming tax corporations or businesses instead of wages?",
      answer:
        "No — Wyoming also levies no corporate income tax, so it isn't simply shifting the burden from " +
        "individuals to businesses. This calculator only covers personal wage withholding, but it's worth " +
        "knowing that Wyoming's low-tax approach extends beyond just individual paychecks.",
    },
  ];
  ...
  const toolContent = {
    ...
    description:
      "This Wyoming income tax calculator and paycheck tax calculator shows you, in one place, exactly what's " +
      "withheld from your paycheck and what you take home. Use it as a general tax calculator for Wyoming, a " +
      "Wyoming payroll tax calculator for federal withholding and FICA, or a salary tax calculator for any pay " +
      "frequency — since Wyoming charges no state income tax, more of every paycheck stays with you.",
  ...
  const seoMetaContent = {
    contentType: "tool",
    metaTitle: "Wyoming Income Tax Calculator (2026) — Paycheck & Payroll Tax",
    metaDescription:
      "Free Wyoming income tax calculator and paycheck tax calculator. Estimate federal income tax, payroll tax " +
      "(FICA), and take-home pay — Wyoming has no state income tax.",
  ...
  console.log(
    "Open it in /admin/tools, review it, then set Status to Published when you're happy with it. " +
      "Its live URL will be /tools/" + SLUG + ". Once published, link it to Wyoming's row in " +
