// One-time (but safe to re-run) setup script: creates the Texas Income Tax
...
//
// Like Nevada, Texas levies NO state income tax on wages, so this tool's
// breakdown always shows a $0 Texas State Income Tax line. The math lives
// in code, not the database: see `customCalculators["texas-tax-calculator"]`
...
//
// HOW TO RUN
//   npx tsx prisma/create-texas-tax-tool.ts
// or
//   npm run db:create-texas-tool

import { PrismaClient, Prisma } from "@prisma/client";

const prisma = new PrismaClient();

const SLUG = "texas-tax-calculator";
...
async function main() {
  ...
  const calcResults = [
    ...
    { key: "stateIncomeTax", label: "Texas State Income Tax", format: "currency" },
  ...
  const instructions =
    "Whether you're looking for a Texas income tax calculator, a paycheck tax calculator, a payroll tax " +
    "calculator, or just a general tax calculator for Texas, this tool covers it: it works out federal income " +
    "tax, Social Security, Medicare, and Texas's state income tax (always zero) from your salary, all in one " +
    "place.\n\n" +
    ...
    "breakdown: gross pay, federal income tax, Social Security tax, Medicare tax, Texas state income tax " +
    "(always $0 — Texas has no state income tax), total deductions, and your estimated take-home pay, both per " +
    "paycheck and for the year.\n\n" +
    "Texas levies no tax on wages, and its state constitution requires voter approval before one could ever be " +
    "introduced. Instead of an income tax, Texas leans more heavily on sales tax and property tax to fund state " +
    "and local government — a structural trade-off worth knowing about even though neither of those taxes comes " +
    "out of a paycheck.\n\n" +
  ...
  const assumptions =
    ...
    "Texas has no state income tax and no local income taxes, so that part of the breakdown needs no " +
    "assumptions — it's always $0 for every filer, at every income level, statewide.\n\n" +
    "This calculator only reflects taxes withheld from a paycheck. It doesn't model Texas's sales tax or its " +
    "comparatively high property taxes, since neither is deducted from wages — they're relevant to a Texas " +
    "household's overall tax burden, just not to what shows up on a pay stub.\n\n" +
  ...
  const examples =
    ...
    "after federal income tax, Social Security, and Medicare. Texas adds no state income tax on top of that.\n\n" +
    "Because Texas has no state income or payroll tax, the only difference between this estimate and a paycheck " +
    "in a state with income tax is that extra state withholding line — everything else (federal income tax, " +
    "Social Security, Medicare) is calculated the same way nationwide.";

  const faq = [
    {
      question: "Does Texas have a state income tax?",
      answer:
        "No. Texas is one of nine U.S. states with no state income tax, so nothing is withheld from your " +
        "paycheck for state income tax — your take-home pay is reduced only by federal income tax and FICA " +
        "(Social Security and Medicare).",
    },
    {
      question: "Why doesn't Texas have an income tax?",
      answer:
        "Texas's state constitution requires voter approval before a personal income tax could be introduced. " +
        "Instead, Texas relies more heavily on sales tax and property tax to fund state and local government.",
    },
    {
      ...
      answer:
        ...
        "on your filing status). There's no state or local income tax in Texas.",
    },
    {
      ...
      answer:
        ...
        "Texas's (zero) state payroll tax, all in the same breakdown.",
    },
    {
      question: "Is there a Texas salary tax calculator I can use for any pay frequency?",
      answer:
        "Yes — this calculator works as a Texas salary tax calculator for weekly, biweekly, semi-monthly, " +
    ...
    {
      question: "What is my take-home pay in Texas?",
      answer:
        "Your take-home pay is your gross salary minus federal income tax, Social Security, Medicare, and any " +
        "deductions you enter. Because Texas has no state income tax, nothing further is subtracted for state " +
        "tax, so Texas workers typically keep more of their gross pay than workers in states that do tax " +
        "income.",
    },
    {
  ...
  const toolContent = {
    ...
    description:
      "This Texas income tax calculator and paycheck tax calculator shows you, in one place, exactly what's " +
      "withheld from your paycheck and what you take home. Use it as a general tax calculator for Texas, a " +
      "Texas payroll tax calculator for federal withholding and FICA, or a salary tax calculator for any pay " +
      "frequency — since Texas charges no state income tax, more of every paycheck stays with you.",
  ...
  const seoMetaContent = {
    contentType: "tool",
    metaTitle: "Texas Income Tax Calculator (2026) — Paycheck & Payroll Tax",
    metaDescription:
      "Free Texas income tax calculator and paycheck tax calculator. Estimate federal income tax, payroll tax " +
      "(FICA), and take-home pay — Texas has no state income tax.",
  ...
  console.log(
    "Open it in /admin/tools, review it, then set Status to Published when you're happy with it. " +
      "Its live URL will be /tools/" + SLUG + ". Once published, link it to Texas's row in " +
