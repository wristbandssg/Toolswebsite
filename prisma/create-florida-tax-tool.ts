// One-time (but safe to re-run) setup script: creates the Florida Income
...
// create-nevada-paycheck-tool.ts / create-alaska-tax-tool.ts's structure
// and template (tool-template-3) exactly, per the established pattern.
//
// Like Nevada and Alaska, Florida levies NO state income tax — confirmed
// via the Tax Foundation's 2026 Florida summary
// (taxfoundation.org/location/florida/): "Florida does not have an
// individual income tax." Florida also has no estate or inheritance tax; it
// does have a state sales tax (6%, unlike Nevada/Alaska which either have
// or don't have one differently) — not part of an income-tax calculator's
// scope, so left out of the tool itself, same as the other zero-income-tax
// state tools.
//
// Because the state math is identical to Nevada's and Alaska's (always
// $0), this script intentionally stays as simple as those — no dependents
// field, no exemption logic. The actual math lives in code, not the
// database: see `customCalculators["florida-tax-calculator"]` in
...
//
// HOW TO RUN
//   npx tsx prisma/create-florida-tax-tool.ts
// or
//   npm run db:create-florida-tool

import { PrismaClient, Prisma } from "@prisma/client";

const prisma = new PrismaClient();

const SLUG = "florida-tax-calculator";
...
async function main() {
  ...
  const calcResults = [
    ...
    { key: "stateIncomeTax", label: "Florida State Income Tax", format: "currency" },
  ...
  const instructions =
    "Whether you're looking for a Florida income tax calculator, a paycheck tax calculator, a payroll tax " +
    "calculator, or just a general tax calculator for Florida, this tool covers it: it works out federal " +
    "income tax, Social Security, Medicare, and Florida's state income tax (always zero) from your salary, " +
    "all in one place.\n\n" +
    ...
    "full breakdown: gross pay, federal income tax, Social Security tax, Medicare tax, Florida state income " +
    "tax (always $0 — Florida has no state income tax), total deductions, and your estimated take-home pay, " +
  ...
  const assumptions =
    ...
    "cafeteria-plan/Section 125-style deduction — some deduction types only reduce one or the other, which " +
    "this calculator doesn't distinguish between.\n\n" +
    "Florida has no state income tax and no local income taxes, so that part of the breakdown needs no " +
    "assumptions — it's always $0 for every filer, at every income level, statewide. (Florida does have a 6% " +
    "state sales tax plus local surtaxes, though that's separate from this calculator's scope.)\n\n" +
  ...
  const examples =
    ...
    "after federal income tax, Social Security, and Medicare. Florida adds no state income tax on top of " +
    "that.\n\n" +
    "Because Florida has no state income or payroll tax, the only difference between this estimate and a " +
  ...
  const faq = [
    {
      ...
      answer:
        "No. Florida is one of a handful of U.S. states with no state income tax, so nothing is withheld from " +
        "your paycheck for state income tax — your take-home pay is reduced only by federal income tax and " +
        "FICA (Social Security and Medicare).",
    },
    {
      question: "What taxes are actually taken out of a Florida paycheck?",
      answer:
        "Federal income tax (based on your income and filing status), Social Security tax (6.2% up to the " +
        "annual wage base), and Medicare tax (1.45%, plus an extra 0.9% on wages above a threshold that " +
        "depends on your filing status). There's no state or local income tax in Florida.",
    },
    {
      ...
      answer:
        ...
        "and this calculator includes all of it: federal income tax, Social Security, and Medicare, plus " +
        "Florida's (zero) state payroll tax, all in the same breakdown.",
    },
    {
      question: "Is there a Florida salary tax calculator I can use for any pay frequency?",
      answer:
        "Yes — this calculator works as a Florida salary tax calculator for weekly, biweekly, semi-monthly, " +
    ...
    {
      question: "What is my after-tax (take-home) pay in Florida?",
      answer:
        "Your after-tax pay is your gross salary minus federal income tax, Social Security, Medicare, and any " +
        "deductions you enter. Because Florida has no state income tax, nothing further is subtracted for " +
        "state tax, so Florida workers typically keep more of their after-tax pay than workers in states that " +
        "do tax income.",
    },
    {
      question: "Does Florida have a state sales tax too?",
      answer:
        "Yes — a 6% state sales tax, plus local county surtaxes in most areas. This calculator covers income " +
        "tax and payroll tax only, not sales tax.",
    },
    {
      question: "How accurate is this calculator?",
      answer:
        "It's an estimate, using the 2026 IRS federal tax brackets and standard deduction amounts. It doesn't " +
        "include tax credits (like the Child Tax Credit), itemized deductions, or every W-4 adjustment, so " +
        "your actual withholding may differ slightly.",
    },
    {
  ...
  const toolContent = {
    ...
    description:
      "This Florida income tax calculator and paycheck tax calculator shows you, in one place, exactly what's " +
      "withheld from your paycheck and what you take home. Use it as a general tax calculator for Florida, a " +
      "Florida payroll tax calculator for federal withholding and FICA, or a salary tax calculator for any pay " +
      "frequency — since Florida charges no state income tax, more of every paycheck stays with you.",
  ...
  const seoMetaContent = {
    contentType: "tool",
    metaTitle: "Florida Income Tax Calculator (2026) — Paycheck & Payroll Tax",
    metaDescription:
      "Free Florida income tax calculator and paycheck tax calculator. Estimate federal income tax, payroll " +
      "tax (FICA), and take-home pay — Florida has no state income tax.",
  ...
  console.log(
    "Open it in /admin/tools, review it, then set Status to Published when you're happy with it. " +
      "Its live URL will be /tools/" + SLUG + ". Once published, link it to Florida's row in " +
